...
describe("projects.yml", () => {
    test("contiene todas las carpetas dashboard-* locales", () => {
        expect(proyectos.length).toBeGreaterThanOrEqual(5);
        expect(carpetasLocales.length).toBe(5);
    ...
    test("destaca al menos un dashboard", () => {
...
describe("generador", () => {
    test("construye el JSON de salida ordenado", () => {
        ...
        expect(json.dashboards.map((j) => j.titulo)).toEqual(
            [...json.dashboards.map((j) => j.titulo)].sort((a, b) => a.localeCompare(b, "es"))
        );
    });
