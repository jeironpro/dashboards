...
const PROBE = "dashboard-administrador";
...
function capturar(id) {
    return new Promise((resolve) => {
        ...
        const perfil = join("/tmp", `db-profile-${id}`);
        const chrome = spawn(
            CHROME,
            [
                "--headless=new",
...
async function main() {
    ...
    console.log(`Capturando ${dashboards.length} dashboards en http://127.0.0.1:${PUERTO}/`);
    const servidor = await servir();
    const ids = dashboards.map((j) => j.id);
